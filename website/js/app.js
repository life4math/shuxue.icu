...
const publishedKnowledgeCache = new Map();
const KNOWLEDGE_CACHE_TTL = 60 * 1000;
...
async function showKnowledgeDetail(id) {
  ...
  if (node.children) {
    renderKnowledgeDetailPanel(node, getStaticKnowledgeDetail(node), { source: 'structure' });
    return;
  }

  const cached = publishedKnowledgeCache.get(node.id);
  if (cached && Date.now() - cached.loadedAt < KNOWLEDGE_CACHE_TTL) {
    renderKnowledgeDetailPanel(node, cached.content, cached.meta);
    return;
  }

  renderKnowledgeDetailPanel(node, {
    title: node.name,
    sections: [{ title: '读取知识正文', items: [{ text: '正在从正式知识库读取已发布版本。' }] }]
  }, { source: 'loading' });

  try {
    const published = await fetchPublishedKnowledge(node.id);
    ...
    if (!published) {
      renderKnowledgeDetailPanel(node, {
        ...
        sections: [{
          title: '正文状态',
          items: [{ text: '该知识点目前没有已发布正文；教师后台中的草稿不会显示在公开页面。' }]
        }]
      }, { source: 'unpublished' });
      return;
    }
    ...
    publishedKnowledgeCache.set(node.id, cachedValue);
    renderKnowledgeDetailPanel(node, cachedValue.content, cachedValue.meta);
  } catch (error) {
    if (requestId !== knowledgeDetailRequestId) return;
    const fallback = getStaticKnowledgeDetail(node, false);
    if (fallback) {
      renderKnowledgeDetailPanel(node, fallback, { source: 'backup' });
      return;
    }
    renderKnowledgeDetailPanel(node, {
      title: node.name,
      sections: [{
        title: '正文暂不可用',
        items: [{ text: '知识库暂时无法连接，请稍后刷新页面重试。' }]
      }]
    }, { source: 'error' });
  }
}

async function fetchPublishedKnowledge(nodeId) {
  const response = await fetch(`/api/v1/public/knowledge/${encodeURIComponent(nodeId)}`, {
  ...
  const payload = knowledge && knowledge.payload;
  if (!payload || !Array.isArray(payload.sections)) {
    throw new Error('knowledge API returned an invalid payload');
  }
  return {
    content: {
      title: String(payload.title || knowledge.title || nodeId),
      sections: payload.sections
    },
    version: Number(knowledge.version || 1),
    publishedAt: knowledge.published_at || null
  };
}

function knowledgeSourceLabel(meta) {
  if (meta.source === 'cms') {
    const date = meta.publishedAt ? ` · ${String(meta.publishedAt).slice(0, 10)}` : '';
    return `正式知识库 · v${meta.version || 1}${date}`;
  }
  if (meta.source === 'loading') return '正在读取正式知识库';
  if (meta.source === 'unpublished') return '尚未发布';
  if (meta.source === 'backup') return '网络异常 · 显示静态备份';
  if (meta.source === 'error') return '知识库暂不可用';
  return '知识目录说明';
}

function renderKnowledgeDetailPanel(node, content, meta = {}) {
  const panel = document.getElementById('knowledge-detail');

  panel.innerHTML = `
    <div class="knowledge-detail-heading">
      <span>${escapeKnowledgeText(node.id)}</span>
      <h3>${escapeKnowledgeText(content.title || node.name)}</h3>
      <p>${node.children ? '展开下级知识点，查看定义、结论和公式。' : '知识正文支持普通文字与 KaTeX 数学公式。'}</p>
      <div class="knowledge-content-status ${escapeKnowledgeText(meta.source || 'structure')}">${escapeKnowledgeText(knowledgeSourceLabel(meta))}</div>
    </div>
      ...
      <div class="detail-info-grid">
        <div class="detail-info-item">
          <div class="detail-info-label">知识点ID</div>
            <div class="detail-info-value">${escapeKnowledgeText(node.id)}</div>
        </div>
        <div class="detail-info-item">
      ...
      <div class="kp-children-list">
        ${node.children.map(child => `
          <button type="button" class="kp-child-item" data-knowledge-id="${child.id}" onclick="selectKnowledgeNode('${child.id}')">
            ...
            <span>${escapeKnowledgeText(child.name)}</span>
...
function getStaticKnowledgeDetail(node, generate = true) {
      ...
      <section class="knowledge-content-section">
        ...
        <div class="knowledge-content-copy">
          ...
          <div class="knowledge-content-items">
            ${(Array.isArray(section.items) ? section.items : []).map(renderKnowledgeItem).join('')}
          </div>
        </div>
      </section>
    `).join('')}
